...
class object_v():
	...
	def __init__(self, embedding, bounding_box, key):
		...
		self.key = key
		self.bdist = 99999
		self.bdist_prev = 99999

	def update(self, embedding, bounding_box, flag = True):
	
		if distance.cdist(bounding_box[np.newaxis,:], self.bounding_box[np.newaxis, :])[0][0] > self.bdist:
			#wrong mapping dude!
			return False
		else:
...
def nearness_mapper(key, out_boxes, embeddings, left_out):
	...
	for ind in left_out:
		if ind1 == ind :
			map_flag = object_sv[key].update(embeddings[ind], out_boxes[ind], True)
			if map_flag:
				flag = True
				left_out.remove(ind)
	...
	if len(new_boxes) == 0:
		...
		for i, key in enumerate(unmapped_objects1):
			...
			if object_sv[key].bdist == -1:
				#object just created
				map_flag = object_sv[key].update(embeddings_cp[corresponding_box_id], out_boxes_cp[corresponding_box_id], True)
				if map_flag:	
