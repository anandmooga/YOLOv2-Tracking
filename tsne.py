...
for frame in video:
	...
	out_boxes2 = np.copy(out_boxes)
	if frame == "frame_0000.jpg":
		out_boxes1 = np.copy(out_boxes)
	...
	for roi in out_boxes:
		...
		for channel in range(activation_shape[3]):
			object_list.append(np.average(activation_map[0, roi[0]-1:roi[2]+1, roi[1]-1:roi[3]+1, channel]))
		embeddings.append(object_list)
		names.append("f" + frame[5:10]+ "_" + class_names[out_classes[z-1]] + "_" +  str(z))
		z += 1
	print(frame)
	if frame == "frame_0001.jpg":
		break

print(out_boxes1)
print(out_boxes2)
print(distance.cdist(out_boxes1, out_boxes2))
print(np.argmin(distance.cdist(out_boxes1, out_boxes2), axis = 1))
