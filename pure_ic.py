...
def insane_conv(prev_img, curr_img, prev_box, scale = 1.05, matplot = True):
...
def tracker(prev_img, curr_img):
	#bounding boxes are currently in object_sv
	for key, value in object_sv.items():
		insane_box = insane_conv(prev_img, curr_img, value.bounding_box)
		value.update(insane_box)
